...
function computeSpeechPosition({ stageEl, speechEl, position }) {
  ...
  const maxLeft = Math.max(
    STAGE_EDGE_PADDING,
    stageRect.width - STAGE_EDGE_PADDING - bubbleRect.width
  );

  const left = onLeftHalf
    ? clamp(
        spriteCenterX + SPEECH_H_GAP,
        STAGE_EDGE_PADDING,
        maxLeft
      )
    : clamp(
        spriteCenterX - SPEECH_H_GAP - bubbleRect.width,
        STAGE_EDGE_PADDING,
        maxLeft
      );
  ...
  const maxTop = Math.max(
    STAGE_EDGE_PADDING,
    stageRect.height - STAGE_EDGE_PADDING - bubbleRect.height
  );

  const top = clamp(
    spriteCenterY - SPEECH_V_OFFSET - bubbleRect.height,
    STAGE_EDGE_PADDING,
    maxTop
  );
...
export function StagePanel({
  ...
  // Prevent crashes if position hasn't been provided yet.
  ...
  const updateSpeechPosition = useCallback(() => {
    ...
    const next = computeSpeechPosition({
      stageEl: stageRef.current,
      speechEl: speechRef.current,
  ...
  }, [safePosition.x, safePosition.y, speech]);
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("resize", updateSpeechPosition);
    };
  }, [updateSpeechPosition]);
  ...
  return (
    <section className="stage-panel">
      ...
      <div className="stage" ref={stageRef}>
        ...
        >
          <img src={cat} width="70" height="70" alt={`${spriteName} sprite`}></img>
        </div>
